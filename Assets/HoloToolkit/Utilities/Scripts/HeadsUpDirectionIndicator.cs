...
namespace HoloToolkit.Unity
{
    ...
    public class HeadsUpDirectionIndicator : MonoBehaviour
    {
        ...
        private FrustumPlanes GetExitPlane(Vector3 targetPosition, Camera camera)
        {
            ...
            Vector3 nearUpperLeft = nearBase + nearTop + nearLeft + cameraPosition;
            Vector3 nearLowerRight = nearBase + nearBottom + nearRight + cameraPosition;
            Vector3 farUpperLeft = farBase + farTop + farLeft + cameraPosition;
            
            Plane d = new Plane(nearUpperLeft, nearLowerRight, farUpperLeft);

            // Calculate the frustum corners needed to create 'e'
            Vector3 nearUpperRight = nearBase + nearTop + nearRight + cameraPosition;
            Vector3 nearLowerLeft = nearBase + nearBottom + nearLeft + cameraPosition;
            Vector3 farUpperRight = farBase + farTop + farRight + cameraPosition;
        ...
        private void UpdatePointerTransform(Camera camera, Plane[] planes, Vector3 targetPosition)
        {
            ...
            Vector3 initialGuess = indicatorPosition;
            ...
            // Scaling the pointer depending on its distance to the target
            float scale = Mathf.Clamp((initialGuess - indicatorPosition).magnitude * 2, 0, 1);
            transform.localScale = Vector3.one * scale;
            pointer.SetActive(Math.Abs(scale) > float.Epsilon && TargetObject.activeSelf);
        }
